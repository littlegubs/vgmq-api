...
export abstract class Duration {
        ...
        try {
            calculateDuration: {
                ...
                while (offset < stat.size) {
                    block = readSync(fd, buffer, 0, 10, offset)
     ...
     * @returns {Number}
     */
    static skipID3v2Tag(buffer: Buffer) {
        if (buffer[0] == 73 && buffer[1] == 68 && buffer[2] == 51) {
            ...
            if ((z0 & 128) == 0 && (z1 & 128) == 0 && (z2 & 128) == 0 && (z3 & 128) == 0) {
                const headerSize = 10
                const tagSize =
                    (z0 & 127) * 2097152 + (z1 & 127) * 16384 + (z2 & 128) * 128 + (z3 & 128)
                const footerSize = buffer[5] & 16 ? 10 : 0
     ...
     */
    static parseFrameHeader(buffer: Buffer) {
        const b1 = buffer[1],
            b2 = buffer[2],
            versionBits = (b1 & 24) >> 3,
