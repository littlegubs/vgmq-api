...
export class lobbyUserStatusAndReconnecting1664370870963 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`lobby_user\` ADD \`isReconnecting\` tinyint NOT NULL DEFAULT 0`);
        await queryRunner.query(`ALTER TABLE \`lobby_user\` CHANGE \`status\` \`status\` enum ('buffering', 'ready_to_play_music') NULL`);
    }

    public async down(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`ALTER TABLE \`lobby_user\` CHANGE \`status\` \`status\` enum ('reconnecting') NULL`);
